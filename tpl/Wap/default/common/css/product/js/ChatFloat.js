...
ChatFloatFactory.prototype = {
    ...
    AjaxRight: function (t, options) {
        ...
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
                ...
                if (obj && obj.weimobid && obj.weimobid.length > 0) {
                    t.initialize(options, obj.weimobid, obj.msgcount, obj.socket);
                    e.setCookie('useright', '{"aid":' + options.AId + ',"weimobid":"' + obj.weimobid + '","socket":' + obj.socket + '}');
                }
            }
        }, function (status) { });
        var openid = options.openid ? options.openid : "";
        ajax.get("../kf/Ajax.aspx?action=userright&AId=" + options.AId + "&openid=" + openid, true);
    },
    SocketMessage: function (AId, weimobid) {
        ...
        ajax.get('../kf/Ajax.aspx?action=GetFloatSocketMessage&weimobid=' + weimobid + "&AId=" + AId, true);
    },
...
window.ChatFloat = function (options) {
    ...
    if (options && options.AId) {
        ...
        if (strStoreData && strStoreData.length > 0) {
            ...
            if (objStorage && objStorage.AId == options.AId && objStorage.weimobid) {
                var socket = 0;
