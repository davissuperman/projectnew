...
Element.prototype.centerX=function(top){
   this.style.left=(_system._scroll().x+_system._zero(_system._client().bw-this.offsetWidth)/2)+"px";
   this.style.top=(top?top:(_system._zero(_system._client().bh-this.offsetHeight)/2))+"px";
};
...
*/   _guide:function(show){
  ...
  /* _guide2:function(){
   ...
   /*_forbidden:function(text){
     //return text.match(/(鑰佸競闀縷钖勭啓鏉钖勫競闀縷娉曡疆鍔�)/)!=null;
      return false;
   }*/
};
(function(){
	 ...
	 if(request.readyState==4){
	     ...
	     if(request.status==200){
		    if(functionName){
	         ...
	         if(loadingMessage != ""){_system._toast("鍙戠敓鎰忓閿欒锛岃绋嶅€欏啀璇�");}
	     }
	 }
    };
