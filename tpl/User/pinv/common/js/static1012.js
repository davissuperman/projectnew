...
var _system={
   ...
   _cover:function(show){
      if(show){
	     $l("cover").style.display="block";
	     $l("cover").style.width=(this._client().bw>this._client().w?this._client().bw:this._client().w)+"px";
	     $l("cover").style.height=(this._client().bh>this._client().h?this._client().bh:this._client().h)+"px";
	  }else{
	     $l("cover").style.display="none";
	  }
   },
   _zero:function(n){
      return n<0?0:n;
