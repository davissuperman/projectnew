...
class StoreAction extends WapAction {
    ...
    public function product() {
        ...
        $normsData = M("new_norms")->where(array('catid' => $product['catid']))->select();
        foreach ($normsData as $row) {
            $normsList[$row['id']] = $row['value'];
        }
        if ($productCatData = M('new_product_cat')->where(array('id' => $product['catid'], 'token' => $this->token))->find()) {
    ...
    public function getCat($carts = '') {
        ...
        if ($obj = M('new_product_setting')->where(array('token' => $this->token))->find()) {
            if ($totalprice >= $obj['price'])
                $mailPrice = 0;
        }
    ...
    public function ordersave() {
        ...
        if ($carts) {
            ...
            $setting = M('new_product_setting')->where(array('token' => $this->token))->find();
            ...
            $row['dining'] = 0;
            $row['score'] = $score;
            //start 查询现在是否是双倍积分的时间 如果是 积分＊2 。如果不是就不乘2
            $set = M('member_jifen')->field('j_jifen,stime,etime')->where(array('token' => $this->token))->find();
            $time = time();
            $jifen = 0;
            if ($time >= $set['stime'] && $time <= $set['etime']) {//双倍积分
                $jifen = (intval($totalprice) * $set["j_jifen"]) * 2;
            } else {
                $jifen = intval($totalprice) * $set["j_jifen"];
            }
            $row['jifen'] = $jifen;
        ...
        if ($normal_rt) {
            ...
            foreach ($carts as $pid => $rowset) {
                ...
                if (is_array($rowset)) {
                    foreach ($rowset as $did => $ro) {
                        M('new_product_detail')->where(array('id' => $did, 'pid' => $pid, 'num' => array('gt', $ro['count'])))->setDec('num', $ro['count']);
                        $total += $ro['count'];
                    }
                } else {
                    $total = $rowset;
            ...
            if ($_POST['saveinfo']) {
                ...
                if ($thisUser) {
                    ...
                    $u = array('u_username' => $row['truename'], 'u_iphone' => $row['tel'], 'u_lxaddress' => $row['address']);
                    $x = M('member_user')->where(array('token' => $this->token, 'openid' => $this->wecha_id))->find();
                    if (empty($x['tel'])) {
                        M('member_user')->where(array('openid' => $this->wecha_id))->save($u);
                    }
