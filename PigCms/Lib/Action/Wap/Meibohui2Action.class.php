...
class MeibohuiAction2 extends BonusAction {


    public $cache;
    public $titleInWeixin = '森田药妆-上海新国际博览中心';
    public $url;
...
//          }
    ...
    public function updatejiangping(){
        $openId = $_GET['openid'];
        $p = M('Meibohui_index')->where(array('openid' => $openId))->find();
        $return = 0;
        if($p){
            M('Meibohui_index')->where(array('id' =>  $p['id'] ))->setInc('lingjiangsum', 1);
            if(1*$p['award'] == 0){
                //更新获奖状态
                $d['id'] = $p['id'];
                $d['award'] = 1;
                M('Meibohui_index')->save($d);
            }
        ...
        if($openId){
            ...
            if($fansInfo){
                ...
                if(isset($_POST['phone']) && $_POST['phone']){
                    ...
                    //是否存在手机号
                    $phone = $_POST['phone'];
                    $p1 = M('Meibohui_index')->where(array('phone' => $phone))->find();
                    $p2 = M('Meibohui_index')->where(array('openid' => $openId))->find();
                    if($p1 || $p2){
                        ...
                        $d['openid'] = $openId;
                        $d['phone'] = $phone;
                        M("Meibohui_index")->add($d);
                        $msg = "提交成功";
                        M("Meibohui_num")->where(array('id' =>1))->setInc('number', 1);
                        $success = true;
                    }
                }else{
