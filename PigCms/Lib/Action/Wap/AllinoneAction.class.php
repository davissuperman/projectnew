...
class AllinoneAction extends SjzAction {
    ...
    public function _initialize() {
        parent :: _initialize();
        $this->url= C('site_url');
        $this->imageUrl = "http://".$this->_server('HTTP_HOST').'/tpl/Wap/default/common/allinone/images/logo1.jpg';
        $this->shareImageUrl = "http://".$this->_server('HTTP_HOST').'/tpl/Wap/default/common/allinone/images/logo1.jpg';
        ...
        if($userOpenId){
            ...
            if($uid){
                //判断此IP是否访问过
                $id = M('allinone_iplist')->where("ip='$ip'")->getField('id');
                if(!$id){
                    $n = array();
                    $n['uid'] = $uid;
                    $n['ip'] = $ip;
                    M('allinone_iplist')->add($n);
                }
            }
        }
    }
    ...
    public function getShareUrl(){
        ...
        return $this->url."/index.php?g=Wap&m=Allinone&a=sharefriend&uid=".$mainId."&gid=$mainGid";
    }
...
//        $userOpenId= "oP9fCtxIGfuDZkYTS9PSzhvZuvcs";
        ...
        if($userOpenId && $fansInfo){
            ...
            if ($code && $state == 'sentian') {
                ...
                $url = urlencode($this->url."/index.php?g=Wap&m=Allinone&a=index&gid=$gid");
...
//        $uniqueViewSql = "SELECT * from tp_allinone_uniqueviewlist where   createtime >= '$start' and createtime<'$end' and fromopenid='$userOpenId' and toopenid='$userOpenId'";
...
//        $userOpenId= 'oP9fCtxIGfuDZkYTS9PSzhvZuvcs';
        ...
        if(!$userOpenId){
            ...
            if ($code && $state == 'sentian') {
                ...
                $url = urlencode($this->url."/index.php?g=Wap&m=Allinone&a=sharefriend&uid=$uid&gid=$gid");
...
//        $this->assign('uploadimagesrc',$uploadImageSrc);
        ...
        $start = date("Y-m-d H:i:s",$start );
        $end = date("Y-m-d H:i:s",$end );
        $uniqueViewSql = "SELECT * from tp_allinone_uniqueviewlist where   createtime >= '$start' and createtime<'$end' and fromopenid='$userOpenId' and toopenid='$MainOpenId'";
        $uniqueViewlist = M('allinone_uniqueviewlist')->query($uniqueViewSql);
        $haveVoted = 1;
        if($uniqueViewlist){
            //不需要增加uniqueviews
        }else{
            M("allinone")->where(array('id' => $info['id']))->setInc('uniqueviews');
            $n = array();
        ...
        //判断当前用户是否已经投过票
        $voteList = M('allinone_votelist')->where(array('fromopenid' => $userOpenId,'toopenid'=>$MainOpenId  ))->find();
        $voteThisUid = 0;
        if($voteList){
            $voteThisUid = 1;
        }
        ...
        $cookieId = M('allinone')->where("openid='$userOpenId'")->getField('id');
        $cookieJoin = 0;
        if($cookieId){
            $cookieJoin = 1;
        }
...
//        $userOpenId= 'oP9fCt-JvXkTShBQIin7jtYF0i6U';
        M("allinone_polldata")->where(array('id' => 1))->setInc('pv');
        if(!$userOpenId){
            $apidata = M('Diymen_set')->where(array('token' => 'rggfsk1394161441'))->find(); //这token 写死了
            $code = trim($_GET["code"]);
            $state = trim($_GET['state']);
            ...
            if ($code && $state == 'sentian') {
                ...
                $url = urlencode($this->url."/index.php?g=Wap&m=Allinone&a=vote&gid=$gid");
                header("location:https://open.weixin.qq.com/connect/oauth2/authorize?appid=" . $apidata['appid'] . "&redirect_uri=$url&response_type=code&scope=snsapi_base&state=sentian#wechat_redirect");
                exit;
            }
        }
        ...
        $this->assign("nonceStr",$noncestr);
        $this->assign("signature",$signature);
        $this->assign("shareurl",$this->url."/index.php?g=Wap&m=Allinone&a=vote");
        $this->assign('gid', $gid);
        ...
        $list = M('allinone_poll')->query( "select * from tp_allinone_poll order by id asc") ;
        $slist = array();
        $savePath = './PUBLIC/imagess/';
        foreach($list as $each){
            $tmp = array();
            $id = $each['uid'];
            $info = M('allinone')->where(array('id' => $id))->find();
            $tmp['name'] = $info['name'];
...
//        $userOpenId= 'oP9fCtxIGfuDZkYTS9PSzhvZuvcs';
        ...
        $gid = $info['gid'];
        $vote = $info['vote'];
//        if($vote >= $this->allinoneCount && isset($info['phone']) && $info['phone']){
//            //redirect
//            header("location:$this->url/index.php?g=Wap&m=Allinone&a=share");
//            exit();
//        }
    ...
    public function getOpenIdByUid($uid){
        $openId = M('allinone')->where("id=$uid")->getField('openid');
        return $openId;
    }
    public function getUidByOpenid($openid){
        $uid = M('allinone')->where("openid='$openid'")->getField('id');
        return $uid;
    }
    public function savePhoneInPage(){
        ...
        //查看当前提交了手机号数码
//        $countNum = M('allinone')->where("phone != ''")->count('id');
//        if($countNum >= 10000){
//            echo 2;
//            return;
//            exit();
//        }
        $id = M('allinone')->where("openid='$fromOpenIdFromPost'")->getField('id');
        //判断此用户是否已经提交了手机号
        $phoneList = M('allinone_phonelist')->where(array('uid' => $id))->find();
        if($phoneList){
            echo 3;
...
//        $fromOpenIdFromPost = "oP9fCtxIGfuDZkYTS9PSzhvZuvcs";
        ...
        $voteList = M('allinone_votelist')->where(array('fromopenid' => $fromOpenIdFromPost,'toopenid'=>$toOpenIdFromPost  ))->find();
        //多次投票
        M("allinone")->where(array('openid' => $toOpenIdFromPost))->setInc('vote');
        if(!$voteList){//
            //投票
            $d = array();
...
//        $fromOpenIdFromPost= 'oP9fCtxIGfuDZkYTS9PSzhvZuvcs';
        ...
        //根据ID取得UID
        $uid = M('allinone_poll')->where("id=$toId")->getField('uid');
        if(!$uid){
            exit();
        }
        $userInfo = M('allinone')->where("id=$uid")->find();
        if(!$userInfo){
            //非法投票
            exit();
        }
...
//        $end = date("Y-m-d H:i:s",$end );
        ...
        $sql = "SELECT id from tp_allinone_polllist where   createtime >= '$start' and createtime<'$end' and fromopenid='$fromOpenIdFromPost'";
        $voteList = M('allinone_polllist')->query($sql);
//        $voteList = M('allinone_polllist')->where(array('fromopenid' => $fromOpenIdFromPost, ))->find();//'toopenid'=>$toOpenIdFromPost
        if(!$voteList){//
            //投票
            $d = array();
            $d['fromopenid'] = $fromOpenIdFromPost;
            $d['toopenid'] = $toOpenIdFromPost;
            $d['createtime'] = time();
            M('allinone_polllist')->add($d);
            M("allinone_poll")->where(array('id' => $toId))->setInc('vote');
            $return = 1;
        }else{
            //已经投过票
            $return = 2;
    ...
    public function saveFormInfo(){
     ...
     //   $userOpenId= 'oP9fCtxIGfuDZkYTS9PSzhvZuvcs';
        ...
        $award = M('allinone_award')->where(array('openid' => $userOpenId))->find();
        if(!$award){
            $m = array();
            $m['openid'] = $userOpenId;
            $m['name'] = $username;
    ...
    public function rank(){
        ...
        if($info['vote'] <$this->allinoneCount){
            //redirect
            header("location:$this->url/index.php?g=Wap&m=Allinone&a=rank1&gid=$gid");
            exit();
        }
        ...
        $uid = $info['id'];
        //查询此用户的排名
        $pValue = M('allinone_phonelist')->where(array('uid' => $uid))->getField('id');
        $vote = $this->allinoneCount;
        $this->assign("vote",$vote);
...
//        $userOpenId= 'oP9fCtxIGfuDZkYTS9PSzhvZuvcs';
    ...
    public function getOrderByOpenId($openId=null){
        $list = M('allinone')->query("select openid, number,share,phonetime from tp_allinone where phone != '' order by number desc,share desc,phonetime asc ");
    ...
    public function award(){
        ...
        $this->assign('selfpage',$this->url."/index.php?g=Wap&m=Allinone&a=sharefriend&gid=$gid");
    ...
    public function setIncViews($id){
        M("allinone")->where(array('id' => $id))->setInc('views');
        $m = array();
        $m['uid'] = $id;
        M('allinone_viewlist')->add($m);
    }
    ...
    public function saveSharePoll(){
        M("allinone_polldata")->where(array('id' => 1))->setInc('sharenumber');
    }
    /*
    * 记录 我也要参加 次数
    */
    public function saveWantJoin(){
        $uid = $_POST['uid'];
        M("allinone")->where(array('id' => $uid ))->setInc('joins');
    }
    ...
    */
    ...
    public function auth(){
        ...
        $url = $this->url."/index.php?g=Wap&m=Allinone&a=index";
        if(!$openId){
            $code =  $_GET['code'];
            $state = $_GET['state'];
            if ($code && $state == 'sentian') {
        ...
        if ($code && $state == 'sentian') {
            if(empty($fansInfo)){
                ...
                $web_access_token = '';
                    ...
                    if(isset($userinfoFromApi['errcode']) && $userinfoFromApi['errcode']){
                        //code 有错误 需要重定向
                        $url = $this->url."/index.php?g=Wap&m=Allinone&a=getOpenId";
                        header("location:$url");
                    }
                    $m['id'] = $apidata['id'];
                    $m['web_access_token'] = $userinfoFromApi['access_token'];
            ...
            $url = urlencode($this->url."/index.php?g=Wap&m=Allinone&a=getOpenId");
            header("location:https://open.weixin.qq.com/connect/oauth2/authorize?appid=" . $apidata['appid'] . "&redirect_uri=$url&response_type=code&scope=snsapi_base&state=sentian#wechat_redirect");
            exit;
        }
    }
