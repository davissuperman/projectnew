...
class SelfformAction extends BaseAction {
    ...
    public function index() {
		...
		$wecha_ids = $this->selfform_value_model->where(array('formid' => $formid,'wecha_id'=>$this->wecha_id))->find();
	//	$wecha_ids[ 'values' ] = unserialize( $wecha_ids[ 'values' ] );
		//echo "<pre>";
		//print_r( $wecha_ids[ 'values' ] );exit;	
        ...
        if (IS_POST) {
            ...
            $row['time'] = time();
            $myid=$this->selfform_value_model->add($row);
            if (cookie('Selfform')) {
                $sd = cookie('Selfform');
                cookie('Selfform', $sd . '_' . $row['formid'], 315360000);
            } else {
                cookie('Selfform', $row['formid'], 315360000);
            }

            $this->redirect(U('Selfform/index', array('token' => $this->token,'myid'=>$myid,  'id' => $thisForm['id'], 'success' => 1)));
        } else {
            //判断是否提交过信息了
            $numck = explode('_', cookie('Selfform'));
            //print_r($numck);
            $submitInfo = NULL;
            if (in_array($formid, $numck)) {
                $submitInfo = $this->selfform_value_model->where(array('formid' => $thisForm['id'],'wecha_id'=>$this->wecha_id))->find();
                if ($submitInfo) {
                    ...
                    $submitted = 1;
                    //二维码图片
                    $imgSrc = generateQRfromGoogle(C('site_url') . '/index.php?g=Wap&m=Selfform&a=submitInfo&token=' . $this->token . '&myid=' . intval($_GET['myid']) . '&id=' . $thisForm['id']);
    ...
    public function submitInfo() {
        ...
        $submitInfo = $this->selfform_value_model->where(array('id' => intval($_GET['myid']), 'formid' => $thisForm['id']))->find();
        if ($submitInfo) {
            $info = unserialize($submitInfo['values']);
            if ($info) {
                foreach ($info as $k => $v) {
