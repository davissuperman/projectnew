<?php
class YuanxiaoAction extends BaseAction{
    ...
    public function yxchouti(){
		    $wecha_id=$_GET['wecha_id'];
		    $token=$_GET['token'];
	        $yuanxiao=M('yuanxiao');
			$rzt=$yuanxiao->where("wecha_id='$wecha_id'")->find();
			if($rzt!=''){
				  $this->assign('rzt',$rzt);
			     //echo '已经抽过了';
			}else{
			$qry=$yuanxiao->where("wecha_id=''")->select();
			...
			foreach($arr as $key => $val){
				$dataAr[] = $qry[$val];
				$sql = "update tp_yuanxiao set wecha_id = '".$wecha_id."' where id = ".$qry[$val]['id'].";";
				$yuanxiao->query($sql);
			}
			...
			$query = 'update tp_yuanxiao set wecha_id = '.$wecha_id.' where id = '.$dataAr[0][id].;
			die;
			var_dump($dataAr);
			
			$yuanxiao->where("id = $dataAr[0][id]")->data($data)->save();
			$yuanxiao->where("id = $dataAr[1][id]")->data($data)->save();
			$yuanxiao->where("id = $dataAr[2][id]")->data($data)->save();
*/

            $this->assign("list",$dataAr);
            }
	        //var_dump($qry);
	...
	public function tj(){
		...
		$yuanxiao=M('yuanxiao');
		$count = 0;
		foreach($postAr['biaoti'] as $key => $val){
			$map['title'] = array('like',"$val%");
			$map['answer'] = $postAr['daan'][$key];
			$result = $yuanxiao->where($map)->select();
			if($result != ''){
				$count = $count+1;
			}
		}
