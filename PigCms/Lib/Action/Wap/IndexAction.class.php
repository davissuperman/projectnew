...
class IndexAction extends BaseAction{
  ...
  KEY `token` (`token`,`taxis`,`display`)
	...
	public function index(){
		...
		//dump($where);
	//	$where['status']=1;
	$flash=M('Flash')->where($where)->order("id asc")->order("orders asc")->select();
         $flash1=M('Flash')->where($where)->find();
	...
	public function getLink($url){
		...
		if (strExists($url,'刮刮卡')){
			...
			//$link='/index.php?g=Wap&m=Card&a=vip&token='.$this->token.'&wecha_id='.$this->wecha_id;//程云注释
		...
		}elseif (strExists($url,'地图')){//程云添加  后台 微网文章分类里面 填写  ‘地图+id 即 （文本 42）/地图’
			if($itemid){
				$link='/index.php?g=Wap&m=Company&a=map&companyid='.$itemid.'&token='.$this->token.'&wecha_id='.$this->wecha_id;	
			}else {
			$link='/index.php?g=Wap&m=Company&a=map&token='.$this->token.'&wecha_id='.$this->wecha_id;
			}		
		}elseif (strExists($url,'文本')){//程云添加  后台 微网文章分类里面 填写  ‘文本+id 即 （文本 42）’
			$link='/index.php?g=Wap&m=Index&a=content&token='.$this->token.'&wecha_id='.$this->wecha_id.'&id='.$itemid;		
		}elseif (strExists($url,'留言墙')){//程云添加 
			$link='/index.php?g=Wap&m=Leave&a=index&token='.$this->token.'&wecha_id='.$this->wecha_id;
		}elseif (strExists($url,'微留言')){//程云添加  
			$link='/index.php?g=Wap&m=Leave&a=index&token='.$this->token.'&wecha_id='.$this->wecha_id;
		}elseif (strExists($url,'年会')){//苗云鹏  
			$link='/index.php?g=Wap&m=Choujiang&a=index&token='.$this->token.'&wecha_id='.$this->wecha_id;
		}else {
			if (strpos($url,'?')){
