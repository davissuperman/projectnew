...
class TuanyuanAction  extends BonusAction {
...
//        var_dump($infoList);
    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            if ($qry) {
                $this->success('操作成功', U('Tuanyuan/index'));
                exit;
            } else {
                $this->error('操作失败');
                exit;
...
//根据GID 得到渠道
        $gidInfo = M('tuanyuan')->where(array('gid' => $gid))->find();
        $infoList = array();
        foreach($info as $each ){
            $tmp = array();
            $tmp['id'] = $each['id'];
...
//        var_dump($infoList);
    ...
    public function addPoll() {
        if($_POST){
            ...
            //判断UID是否存在
            $id = M('tuanyuan_poll')->where("uid='$uid'")->getField('id');
            if(!$id){
                $arr = array();
                $arr['uid'] = $uid;
                M('tuanyuan_poll')->add($arr);
            }
        }
        $db = M('tuanyuan_poll');
        $count = $db->count();
    ...
    public function deletePoll(){
        $id = $_POST['id'];
        M('tuanyuan_poll')->where("id = $id")->delete();
        echo 1;
    }
...
award.address as addres,award.orderid as orderid,award.username as username from tp_bonus_info as info
    ...
    public function export() {
        ...
        $pL =array();
        if($awardFromPost){
            $phoneList = M('tuanyuan_phonelist')->query("SELECT * FROM `tp_tuanyuan_phonelist` group by uid order by id");
            $arr = array();
...
//                }
                ...
                $condition['openid'] = $each['openid'];
                $resAwardList = M('tuanyuan_award')->where($condition)->select();
                $tmp['username'] = null;
                $tmp['userphone'] = null;
            ...
            //全部的数据
            $db = M('tuanyuan');
            $sql = "select *  from tp_tuanyuan where createtime>=$start and createtime<$end order by  createtime asc ";//where gid=$gid limit 100
            $list = M()->query($sql);
            foreach($list as $key => $each){
                $tmp = null;
                $tmp = $each;
                ...
                $phone = $each['phone'];
                $orderId = null;
                $orderId = M('tuanyuan_phonelist')->where(array('uid' => $each['id']) )->getField('id');
                $tmp['orderid'] = $orderId;

                $level = null;
                if(strrchr((string)"$orderId","16") == "16"){
                    $level = 1;
                }elseif(strrchr((string)"$orderId","1") == '1'){
                    $level = 2;
                }elseif(strrchr((string)"$orderId","6") == '6'){
                    $level = 2;
                }
                $tmp['level'] = $level;
                $listArr[] = $tmp;
    ...
    public function secondexport() {
        ...
        $pL =array();
            //全部的数据
            $db = M('tuanyuan_poll');
            $sql = "SELECT p.vote as poll, t.* FROM `tp_tuanyuan_poll` as p left join tp_tuanyuan as t on (t.id=p.uid) order by p.vote desc";//where gid=$gid limit 100
            $list = M()->query($sql);
            foreach($list as $key => $each){
                $tmp = null;
                $tmp = $each;
    ...
    public function exportDataReport(){
        ...
        while($i<35){
            ...
            $queryGidCount = "SELECT * from tp_tuanyuan where   createtime >= $eachFrom and createtime<$eachEnd";
            $list = M('tuanyuan')->query($queryGidCount);
    ...
    public function datareport2(){
        ...
        while($m<35){
            ...
            $queryGidCount = "SELECT gid, count(id)  as countnumber from tp_tuanyuan where  createtime >= $eachFrom2 and createtime<$eachEnd2  group by gid ";
            $list2 = M('tuanyuan')->query($queryGidCount);
    ...
    public function getAwardList(){
        ...
        $phoneList = M('tuanyuan_phonelist')->select();
        $arr = array();
        foreach($phoneList as $each){
            $id = $each['id'];
            $uid = $each['uid'];
            if(strrchr((string)"$id","16") == "16"){
                $arr[] = $uid;
            }elseif(strrchr((string)"$id","1") == '1'){
                $arr[] = $uid;
            }elseif(strrchr((string)"$id","6") == '6'){
