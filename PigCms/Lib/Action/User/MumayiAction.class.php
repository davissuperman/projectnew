...
class MumayiAction  extends BonusAction {
    ...
    public function index() {
        ...
        $count = M('Mumayi_index')->count();
        $page = new Page($count, 25);
        $list = M('Mumayi_index')->query(
            "SELECT * from tp_mumayi_index  order by createtime desc limit $page->firstRow,$page->listRows"); //第二名和你最近的
...
//        $end = $_POST['end'];
        ...
        $start=$start-1;
        $list = M('Mumayi_index')->query(
            "SELECT h.*,f.nickname as name from tp_mumayi_index as h
