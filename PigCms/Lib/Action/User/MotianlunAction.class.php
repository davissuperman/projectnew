...
//根据GID 得到渠道
        $gidInfo = M('motianlun')->where(array('gid' => $gid))->find();
        $infoList = array();
        foreach($info as $each ){
            $tmp = array();
            $tmp['id'] = $each['id'];
...
//        var_dump($infoList);
    ...
    public function addPoll() {
        if($_POST){
            ...
            //判断UID是否存在
            $id = M('motianlun_poll')->where("uid='$uid'")->getField('id');
            if(!$id){
                $arr = array();
                $arr['uid'] = $uid;
                M('motianlun_poll')->add($arr);
            }
        }
        $db = M('motianlun_poll');
        $count = $db->count();
    ...
    public function deletePoll(){
        $id = $_POST['id'];
        M('motianlun_poll')->where("id = $id")->delete();
        echo 1;
    }
...
award.address as addres,award.orderid as orderid,award.username as username from tp_bonus_info as info
    ...
    public function getUserInfo($id){
        $r = M('motianlun')->where("id=$id")->find();
        if(!$r){
            return null;
        }
        return $r;
    ...
    public function datareportall() {
        ...
        if($awardFromPost){
            $db = M('motianlun');
            $sql = "select *  from tp_motianlun where createtime>=$start and createtime<$end  and prize >= 1 and prize <=2 order by  createtime asc ";//where gid=$gid limit 100
            $list = $db->query($sql);
            //  $count = count($list);
            foreach($list as $key => $each){
                $tmp = null;
                ...
                $condition['openid'] = $each['openid'];
                $resAwardList = M('motianlun_award')->where($condition)->select();
                $tmp['username'] = null;
                $tmp['userphone'] = null;
            ...
            //全部的数据
            $db = M('motianlun');
            $sql = "select *  from tp_motianlun where createtime>=$start and createtime<$end order by  createtime asc ";//where gid=$gid limit 100
            $list = M()->query($sql);
            //  $count = count($list);
            foreach($list as $key => $each){
                $tmp = null;
    ...
    public function export() {
        ...
        $pL =array();
        if($awardFromPost){
            $phoneList = M('motianlun_phonelist')->query("SELECT * FROM `tp_motianlun_phonelist` group by uid order by id");
            $arr = array();
...
//                }
            ...
            //全部的数据
            $db = M('motianlun');
            $sql = "select *  from tp_motianlun where createtime>=$start and createtime<$end and  gid=$gid order by  createtime asc ";//where gid=$gid limit 100
            $list = M()->query($sql);
          //  $count = count($list);
            foreach($list as $key => $each){
                $tmp = null;
    ...
    public function secondexport() {
        ...
        $pL =array();
            //全部的数据
            $db = M('motianlun_poll');
            $sql = "SELECT p.vote as poll, t.* FROM `tp_motianlun_poll` as p left join tp_motianlun as t on (t.id=p.uid) order by p.vote desc";//where gid=$gid limit 100
            $list = M()->query($sql);
            foreach($list as $key => $each){
                $tmp = null;
                $tmp = $each;
    ...
    public function exportDataReport(){
        ...
        while($i<35){
            ...
            $queryGidCount = "SELECT * from tp_motianlun where   createtime >= $eachFrom and createtime<$eachEnd";
    ...
    public function datareport2(){
        ...
        while($m<35){
            ...
            $queryGidCount = "SELECT gid, count(id)  as countnumber from tp_motianlun where  createtime >= $eachFrom2 and createtime<$eachEnd2  group by gid ";
            $list2 = M('motianlun')->query($queryGidCount);
    ...
    public function getAwardList(){
        ...
        $phoneList = M('motianlun_phonelist')->select();
        $arr = array();
...
//            $phoneList = M('motianlun_phonelist')->where( $arr)->find();
            $phoneList = M('motianlun_phonelist')->query( "select * from tp_motianlun_phonelist where phone = '$phone' or uid = $uid") ;
            if(!$phoneList){
                $n = array();
                $t = time();
                $n['uid'] = $uid;
