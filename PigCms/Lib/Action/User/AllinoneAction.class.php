...
class AllinoneAction  extends BonusAction {
    ...
    public function index() {
        //2 - Allinone
        $db = M('npic_twocode');
        $where = array('token' => $this->token,'channel_type'=>4);
...
//根据GID 得到渠道
        $gidInfo = M('allinone')->where(array('gid' => $gid))->find();
        $infoList = array();
        foreach($info as $each ){
            $tmp = array();
            $tmp['id'] = $each['id'];
...
//        var_dump($infoList);
    ...
    public function addPoll() {
        if($_POST){
            ...
            //判断UID是否存在
            $id = M('allinone_poll')->where("uid='$uid'")->getField('id');
            if(!$id){
                $arr = array();
                $arr['uid'] = $uid;
                M('allinone_poll')->add($arr);
            }
        }
        $db = M('allinone_poll');
        $count = $db->count();
    ...
    public function deletePoll(){
        $id = $_POST['id'];
        M('allinone_poll')->where("id = $id")->delete();
        echo 1;
    }
...
//                }
    ...
    public function secondexport() {
        ...
        $pL =array();
            //全部的数据
            $db = M('allinone_poll');
            $sql = "SELECT p.vote as poll, t.* FROM `tp_allinone_poll` as p left join tp_allinone as t on (t.id=p.uid) order by p.vote desc";//where gid=$gid limit 100
            $list = M()->query($sql);
            foreach($list as $key => $each){
                $tmp = null;
                $tmp = $each;
    ...
    public function exportDataReport(){
        ...
        while($i<35){
            ...
            $queryGidCount = "SELECT * from tp_allinone where   createtime >= $eachFrom and createtime<$eachEnd";
            $list = M('allinone')->query($queryGidCount);
    ...
    public function datareport2(){
        ...
        while($m<35){
            ...
            $queryGidCount = "SELECT gid, count(id)  as countnumber from tp_allinone where  createtime >= $eachFrom2 and createtime<$eachEnd2  group by gid ";
            $list2 = M('allinone')->query($queryGidCount);
...
//        echo strrchr("1019","8");
    ...
    public function getAwardList(){
        ...
        $phoneList = M('allinone_phonelist')->select();
        $arr = array();
...
//            $phoneList = M('allinone_phonelist')->where( $arr)->find();
            $phoneList = M('allinone_phonelist')->query( "select * from tp_allinone_phonelist where phone = '$phone' or uid = $uid") ;
            if(!$phoneList){
                $n = array();
                $t = time();
                $n['uid'] = $uid;
