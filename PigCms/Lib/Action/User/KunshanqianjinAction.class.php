...
class KunshanqianjinAction  extends BonusAction {
    ...
    public function index() {
        ...
        $count = M('Kunshanqianjin_index')->count();
        $page = new Page($count, 25);
        $list = M('Kunshanqianjin_index')->query(
            "SELECT * from tp_kunshanqianjin_index  order by createtime desc limit $page->firstRow,$page->listRows"); //第二名和你最近的
...
//        $end = $_POST['end'];
        ...
        $start=$start-1;
        $list = M('Kunshanqianjin_index')->query(
            "SELECT h.*,f.nickname as name from tp_kunshanqianjin_index as h
