...
class AnniversaryAction  extends BonusAction {
...
//        var_dump($infoList);
    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            if ($qry) {
                $this->success('操作成功', U('Anniversary/index'));
                exit;
            } else {
                $this->error('操作失败');
                exit;
...
//根据GID 得到渠道
        $gidInfo = M('anniversary')->where(array('gid' => $gid))->find();
        $infoList = array();
        foreach($info as $each ){
            $tmp = array();
            $tmp['id'] = $each['id'];
...
//        var_dump($infoList);
    ...
    public function addPoll() {
        if($_POST){
            ...
            //判断UID是否存在
            $id = M('anniversary_poll')->where("uid='$uid'")->getField('id');
            if(!$id){
                $arr = array();
                $arr['uid'] = $uid;
                M('anniversary_poll')->add($arr);
            }
        }
        $db = M('anniversary_poll');
        $count = $db->count();
        $page = new Page($count, 25);
    ...
    public function deletePoll(){
        $id = $_POST['id'];
        M('anniversary_poll')->where("id = $id")->delete();
        echo 1;
    }
...
award.address as addres,award.orderid as orderid,award.username as username from tp_bonus_info as info
    ...
    public function getUserInfo($id){
        $r = M('anniversary')->where("id=$id")->find();
        if(!$r){
            return null;
        }
        return $r;
...
//                }
                ...
                $condition['openid'] = $each['openid'];
                $resAwardList = M('anniversary_award')->where($condition)->select();
                $tmp['username'] = null;
                $tmp['userphone'] = null;
            ...
            //全部的数据
            $db = M('anniversary');
            $sql = "select *  from tp_anniversary where createtime>=$start and createtime<$end order by  createtime asc ";//where gid=$gid limit 100
            $list = M()->query($sql);
            foreach($list as $key => $each){
                $tmp = null;
                $tmp = $each;
    ...
    public function secondexport() {
        ...
        $pL =array();
            //全部的数据
            $db = M('anniversary_poll');
            $sql = "SELECT p.vote as poll, t.* FROM `tp_anniversary_poll` as p left join tp_anniversary as t on (t.id=p.uid) order by p.vote desc";//where gid=$gid limit 100
            $list = M()->query($sql);
            foreach($list as $key => $each){
                $tmp = null;
                $tmp = $each;
    ...
    public function exportDataReport(){
        ...
        while($i<35){
            ...
            $queryGidCount = "SELECT * from tp_anniversary where   createtime >= $eachFrom and createtime<$eachEnd";
            $list = M('anniversary')->query($queryGidCount);
    ...
    public function datareport2(){
        ...
        while($m<35){
            ...
            $queryGidCount = "SELECT gid, count(id)  as countnumber from tp_anniversary where  createtime >= $eachFrom2 and createtime<$eachEnd2  group by gid ";
            $list2 = M('anniversary')->query($queryGidCount);
...
//            $phoneList = M('anniversary_phonelist')->where( $arr)->find();
            $phoneList = M('anniversary_phonelist')->query( "select * from tp_anniversary_phonelist where phone = '$phone' or uid = $uid") ;
            if(!$phoneList){
                $n = array();
                $t = time();
                $n['uid'] = $uid;
