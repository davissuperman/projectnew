...
class ShijiazhuangAction  extends BonusAction {
    ...
    public function index() {
        ...
        $count = M('shijiazhuang_index')->count();
        $page = new Page($count, 25);
        $list = M('shijiazhuang_index')->query(
            "SELECT * from tp_shijiazhuang_index  order by createtime desc limit $page->firstRow,$page->listRows"); //第二名和你最近的
...
//        $end = $_POST['end'];
        ...
        $start=$start-1;
        $list = M('shijiazhuang_index')->query(
            "SELECT h.*,f.nickname as name from tp_shijiazhuang_index as h
