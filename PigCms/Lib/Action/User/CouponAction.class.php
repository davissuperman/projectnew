...
class CouponAction extends UserAction{
	...
	//dump(session('token'));
		...
		$count = M('Lottery')->where(array('token'=>session('token'),'type'=>3))->count();
		$page = new Page( $count,15 );
		$show = $page->show();
		$list = M('Lottery')->where(array('token'=>session('token'),'type'=>3))->limit( $page->firstRow.','.$page->listRows )->select();
