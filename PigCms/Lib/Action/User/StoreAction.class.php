...
class StoreAction extends UserAction {
    ...
    public function catAdd() {
        if (IS_POST) {
            $this->insert('new_product_cat', '/index');
        } else {
    ...
    public function catSet() {
        $id = $this->_get('id');
        $checkdata = M('new_product_cat')->where(array('id' => $id))->find();
        if (empty($checkdata)) {
    ...
    public function norms() {
        ...
        if ($checkdata = M('new_product_cat')->where(array('id' => $catid, 'token' => session('token')))->find()) {
    ...
    public function normsAdd() {
        ...
        if ($data = M('new_product_cat')->where(array('id' => $this->_get('catid'), 'token' => session('token')))->find()) {
        ...
        if (IS_POST) {
            ...
            $data = M('new_norms')->where(array('id' => $this->_get('id'), 'type' => $type, 'catid' => $this->_get('catid')))->find();
    ...
    public function attributeAdd() {
        if ($checkdata = M('new_product_cat')->where(array('id' => $this->_get('catid'), 'token' => session('token')))->find()) {
            $this->assign('catData', $checkdata);
        } else {
            $this->error("没有选择相应的分类.", U('Store/index'));
        }
        if (IS_POST) {
            $data = D('new_attribute');
            $id = intval($this->_post('id'));
            ...
            $data = M('new_attribute')->where(array('id' => $this->_get('id'), 'token' => session('token'), 'catid' => $this->_get('catid')))->find();
            $this->assign('catid', $this->_get('catid'));
            $this->assign('token', session('token'));
            $this->assign('set', $data);
            $this->display();
    ...
    public function addNew() {
        ...
        //产品的规格
        $normsData = M("new_norms")->where(array('catid' => $catid))->select();
        ...
        if ($id && ($product = M('new_product')->where(array('catid' => $catid, 'token' => session('token'), 'id' => $id))->find())) {
            ...
            //分类产品的属性
            $data = M("new_attribute")->where(array('catid' => $catid))->select();
            $attributeData = array();
            foreach ($data as $row) {
                $row['aid'] = $row['id'];
                $row['id'] = 0;
    ...
    public function del() {
        ...
        if (IS_GET) {
            ...
            if ($back == true) {
                ...
                $this->success('操作成功', U('Store/product', array('token' => session('token'), 'catid' => $catid, 'dining' => $this->isDining)));
            } else {
                $this->error('服务器繁忙,请稍后再试', U('Store/product', array('token' => session('token'), 'catid' => $catid)));
            }
        }
    }
    ...
    public function orderInfo() {
        ...
        if (IS_POST) {
            ...
            //订单信息同步到CRM
            if ($_POST['sent'] == 1) {
                //如果是发货就把订单记录到crm的消费记录
                $user = D('member_user')->where(array('token' => $thisOrder['token'], 'openid' => $thisOrder['wecha_id']))->find();
                //如果是发货就把订单记录到crm的消费记录
                $money = $user['u_money'] + $thisOrder['price'];
                $jifen = $user['u_number'] + $thisOrder['jifen'];
                M('member_user')->where(array('uid' => $user['uid']))->save(array('u_money' => $money, 'u_number' => $jifen, 'u_member' => 4,'consumption'=>  time()));
                //查询规则
            }
            if ($_POST['sent'] == 2) {//删除crm中的数据同时减去积分
                $user = D('member_user')->where(array('token' => $thisOrder['token'], 'openid' => $thisOrder['wecha_id']))->find();
                $money = $user['u_money'] - $thisOrder['price']; //用户消费金额修改
                $jifen = $user['u_number'] - $thisOrder['jifen']; //用户积分修改
                $type = 4;
                if ($jifen <= 0) {
                    $type = 3;
                } else {
                ...
                M('member_user')->where(array('uid' => $user['uid']))->save(array('u_money' => $money, 'u_number' => $jifen, 'u_member' => $type));
                // M('member_xfmoney')->where(array('token' => $thisOrder['token'], 'openid' => $thisOrder['wecha_id'], 'x_number' => $thisOrder['orderid']))->delete();
            }
            ...
            $product_diningtable_model = M('new_product_cat');
...
//			print_r($list);die;
    ...
    public function getCat($carts) {
        ...
        $catlist = $norms = array();
        if ($cartIds) {
            $normsdata = M('new_norms')->where(array('catid' => array('in', $cartIds)))->select();
        ...
        foreach ($carts as $pid => $rowset) {
            ...
            if (is_array($rowset)) {
                ...
                if ($dids) {
                    $temp = M('new_product_detail')->where(array('id' => array('in', $dids), 'pid' => $pid))->select();
                    foreach ($temp as $row) {
        ...
        if (count($ids)) {
            ...
            foreach ($tmp as $row) {
                if (isset($detail_list[$row['id']])) {
                    if ($catData = M('new_product_cat')->where(array('id' => $row['catid']))->find()) {
        ...
        foreach ($carts as $pid => $rowset) {
            ...
            if (is_array($rowset)) {
                ...
                $normsdata = M('new_pnorms')->where(array('catid' => $tmp['catid']))->select();
                foreach ($normsdata as $r) {
                    $norms[$r['id']] = $r['value'];
                }
                if ($dids) {
