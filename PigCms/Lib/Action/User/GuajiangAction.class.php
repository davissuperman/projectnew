...
class GuajiangAction extends UserAction{
	public function index(){
		...
		$count = M('Lottery')->field('id,title,joinnum,click,keyword,statdate,enddate,status')->where(array('token'=>session('token'),'type'=>2))->count();
		$page = new Page( $count,15 );
		$show = $page->show();
		$list=M('Lottery')->field('id,title,joinnum,click,keyword,statdate,enddate,status')->where(array('token'=>session('token'),'type'=>2))->limit( $page->firstRow.','.$page->listRows )->select();
		//dump($list);
	...
	public function sn(){
		...
		$count = M('Lottery_record')->where('token="'.session('token').'" and lid='.$id.' and sn!=""')->count();
		$page = new Page( $count,15 );
		$show = $page->show();
		$record = M('Lottery_record')->where('token="'.session('token').'" and lid='.$id.' and sn!=""')->limit( $page->firstRow.','.$page->listRows )->select();
