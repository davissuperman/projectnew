...
class LotteryAction extends UserAction{
	public function index(){
		...
		$count      = M('Lottery')->where(array('token'=>session('token'),'type'=>1))->count();
		$Page       = new Page($count,15);
		$show       = $Page->show();
		$list = M('Lottery')->where(array('token'=>session('token'),'type'=>1))->limit($Page->firstRow.','.$Page->listRows)->select();	
			...
			if($_POST['enddate'] < $_POST['statdate']){
				...
				if($data->create()!=false){
					if($id=$data->add()){
						...
						$this->success('创建成功',U('Lottery/index'));
					}else{
