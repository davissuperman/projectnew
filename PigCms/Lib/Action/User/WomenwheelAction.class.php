...
//根据GID 得到渠道
        $gidInfo = M('womenwheel')->where(array('gid' => $gid))->find();
        $infoList = array();
        foreach($info as $each ){
            $tmp = array();
            $tmp['id'] = $each['id'];
            ...
            $firstDrawTime = M('womenwheel_drawlist')->where('uid='. $each['id'] ." and position=1")->order('id desc')->getField('createtime');
            $tmp['first_draw_time'] = $firstDrawTime;

            $tmp['second_draw'] = $each['rank2'];
            $secondDrawTime = M('womenwheel_drawlist')->where('uid='. $each['id'] ." and position=2")->order('id desc')->getField('createtime');
...
//        var_dump($infoList);
    ...
    public function addPoll() {
        if($_POST){
            ...
            //判断UID是否存在
            $id = M('womenwheel_poll')->where("uid='$uid'")->getField('id');
            if(!$id){
                $arr = array();
                $arr['uid'] = $uid;
                M('womenwheel_poll')->add($arr);
            }
        }
        $db = M('womenwheel_poll');
        $count = $db->count();
        $page = new Page($count, 25);
    ...
    public function deletePoll(){
        $id = $_POST['id'];
        M('womenwheel_poll')->where("id = $id")->delete();
        echo 1;
    }
...
award.address as addres,award.orderid as orderid,award.username as username from tp_bonus_info as info
    ...
    public function getUserInfo($id){
        $r = M('womenwheel')->where("id=$id")->find();
        if(!$r){
            return null;
        }
        return $r;
    ...
    public function datareportall() {
        ...
        if($awardFromPost){
            $db = M('womenwheel');
            $sql = "select *  from tp_womenwheel where createtime>=$start and createtime<$end  and prize >= 1 and prize <=2 order by  createtime asc ";//where gid=$gid limit 100
            $list = $db->query($sql);
            //  $count = count($list);
            foreach($list as $key => $each){
                $tmp = null;
                ...
                $condition['openid'] = $each['openid'];
                $resAwardList = M('womenwheel_award')->where($condition)->select();
                $tmp['username'] = null;
                $tmp['userphone'] = null;
                ...
                $phone = $each['phone'];
                $orderId = null;
                $orderId = M('womenwheel_phonelist')->where(array('uid' => $each['id']) )->getField('id');
                $tmp['orderid'] = $orderId;
            ...
            //全部的数据
            $db = M('womenwheel');
            $sql = "select *  from tp_womenwheel where createtime>=$start and createtime<$end order by  createtime asc ";//where gid=$gid limit 100
            $list = M()->query($sql);
            //  $count = count($list);
            foreach($list as $key => $each){
                $tmp = null;
    ...
    public function export() {
        ...
        $pL =array();
        if($awardFromPost){
            $phoneList = M('womenwheel_phonelist')->query("SELECT * FROM `tp_womenwheel_phonelist` group by uid order by id");
            $arr = array();
...
//                }
            ...
            //全部的数据
            $db = M('womenwheel');
            $sql = "select *  from tp_womenwheel where createtime>=$start and createtime<$end and  gid=$gid order by  createtime asc ";//where gid=$gid limit 100
            $list = M()->query($sql);
          //  $count = count($list);
            foreach($list as $key => $each){
                $tmp = null;
    ...
    public function secondexport() {
        ...
        $pL =array();
            //全部的数据
            $db = M('womenwheel_poll');
            $sql = "SELECT p.vote as poll, t.* FROM `tp_womenwheel_poll` as p left join tp_womenwheel as t on (t.id=p.uid) order by p.vote desc";//where gid=$gid limit 100
            $list = M()->query($sql);
            foreach($list as $key => $each){
                $tmp = null;
                $tmp = $each;
    ...
    public function exportDataReport(){
        ...
        while($i<35){
            ...
            $queryGidCount = "SELECT * from tp_womenwheel where   createtime >= $eachFrom and createtime<$eachEnd";
            $list = M('womenwheel')->query($queryGidCount);
    ...
    public function datareport2(){
        ...
        while($m<35){
            ...
            $queryGidCount = "SELECT gid, count(id)  as countnumber from tp_womenwheel where  createtime >= $eachFrom2 and createtime<$eachEnd2  group by gid ";
            $list2 = M('womenwheel')->query($queryGidCount);
    ...
    public function getAwardList(){
        ...
        $phoneList = M('womenwheel_phonelist')->select();
        $arr = array();
...
//            $phoneList = M('womenwheel_phonelist')->where( $arr)->find();
            $phoneList = M('womenwheel_phonelist')->query( "select * from tp_womenwheel_phonelist where phone = '$phone' or uid = $uid") ;
            if(!$phoneList){
                $n = array();
                $t = time();
                $n['uid'] = $uid;
