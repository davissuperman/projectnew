...
/*
  ...
  $Id: upload.class.php 2 2013-12-19 02:27:13Z 53623075 $
 */
...
Class upload {
    ...
    function execute($userdir) {
        $arr = array();
        $keys = array_keys($_FILES["file"]["name"]);
        foreach ($keys as $key) {
            ...
            $arr[$key]['file'] = $userdir . '/' . $filename;
            $arr[$key]['name'] = $_FILES['file']['name'][$key];
            $arr[$key]['type'] = $_FILES['file']['type'][$key];
            $arr[$key]['size'] = $_FILES['file']['size'][$key];
    ...
    function copy($sourcefile, $destfile) {
        $x=move_uploaded_file($sourcefile, $destfile);
        @unlink($sourcefile);
    }
